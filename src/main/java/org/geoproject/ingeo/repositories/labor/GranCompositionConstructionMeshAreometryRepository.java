...
//            "WHERE s.point_id = :surveyPointId ORDER BY s.labor_number")
    ...
    List<GranCompositionConstructionMeshAreometry> findBySurveyPoint(Long surveyPointId);

    @Query("SELECT g FROM GranCompositionConstructionMeshAreometry g " +
            "WHERE g.sample.laborNumber IN (:dtoLaborNumbers)")
    List<GranCompositionConstructionMeshAreometry> findAllByLaborNumberIn(List<String> dtoLaborNumbers);
}